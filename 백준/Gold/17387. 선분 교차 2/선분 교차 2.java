...
public class Main {
    ...
    public static boolean isIntersect(Point a, Point b, Point c, Point d){
        ...
        // 두 선분이 일직선 상에 놓여 있는 경우
        if(l1 == 0 && l2 == 0){
            // 두 선분이 일직선 상에서 떨어져있다면 false 반환
            if(a.x > c.x && a.x > d.x && b.x > c.x && b.x > d.x) return false;
            else if(a.x < c.x && a.x < d.x && b.x < c.x && b.x < d.x) return false;
            else if(a.y > c.y && a.y > d.y && b.y > c.y && b.y > d.y) return false;
            else if(a.y < c.y && a.y < d.y && b.y < c.y && b.y < d.y) return false;
            // 두 선분이 일직선 상에서 포개어진다면 true 반환
            else return true;
        }
